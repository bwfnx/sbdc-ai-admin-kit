...
2. **Check each result before advisors touch it.** Score it against [STANDARD.md](STANDARD.md) (ten pass/fail lines), then run it cold in a fresh chat with [TESTING.md](TESTING.md). A migration always produces *something*; the question is whether it still asks the right questions, refuses what it should, and gives you your format.
...
**gpt-to-skill.** Install: in ChatGPT, switch the chat to **Work mode** (a regular chat can't install from GitHub) and say `install https://github.com/bwfnx/sbdc-ai-admin-kit`; Codex takes the same sentence. Or download this repo as a zip and upload `skills/gpt-to-skill` at chatgpt.com/skills. Claude Code: `/plugin marketplace add bwfnx/sbdc-ai-admin-kit` then `/plugin install sbdc-ai-admin-kit@sbdc-ai-admin`. Anywhere else: paste `skills/gpt-to-skill/SKILL.md` into a project's instructions.
- Export your GPTs into one `my-gpts.md`: for each, a `## GPT name` heading, then Description, Conversation starters, Knowledge file names, Capabilities, and the whole Instructions box. Markdown or plain text is best; Word works but adds nothing, and one file avoids upload limits.
- **Already copied each GPT into its own Word file?** Don't re-paste. Name each file after its GPT, put them in one folder, and run `py tools/gpts-to-md.py "C:\Users\you\Desktop\My GPTs"`. It writes `my-gpts.md` next to the folder, one section per file, and lists anything it couldn't read. It runs on your computer; nothing is uploaded. No Python? In ChatGPT Work mode, Codex, or Claude, say: "run tools/gpts-to-md.py from the sbdc-ai-admin-kit repo on my GPTs folder."
